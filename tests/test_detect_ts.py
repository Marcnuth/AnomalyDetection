...
class TestAnomalyDetection(unittest.TestCase):
    ...
    def test__detect_anoms_pos(self):
        shesd = _detect_anoms(self.data1, k=0.02, alpha=0.05,
                                ...
                                use_decomp=True, use_esd=False,
                                direction='pos')
        self.assertEqual(50, len(shesd['anoms']))
    ...
    def test_anomaly_detect_ts_pos_only(self):
        results = anomaly_detect_ts(self.data1, max_anoms=0.02,
                                      direction='pos',
                                      only_last=None, plot=False)
        self.assertEqual(50, len(results['anoms']))
    ...
    def test_get_data_tuple(self):
        ...
        self.assertEqual('min', granularity)
        self.assertEqual(14398, len(raw_data))

    def test_get_max_outliers(self):
        self.assertEqual(719, _get_max_outliers(self.data1, 0.05))
    ...
    def test_get_decomposed_data_tuple(self):
        ...
        self.assertEqual(14398, len(data))
        self.assertEqual(14398, len(smoothed_data))
    ...
    def test_get_plot_breaks(self):
        self.assertEqual(36, _get_plot_breaks('day', 'day'))
        self.assertEqual(12, _get_plot_breaks('min', 'day'))
        self.assertEqual(3, _get_plot_breaks('min', 'min'))
    ...
    def test_plot_data_files(self):
        results = anomaly_detect_ts(self.data1,
                                              direction='both', alpha=0.02, max_anoms=0.35,
                                              plot=True, longterm=True)
        self.assertEqual("count", results['plot'].get_title()) #just checking that the plot was returned for the test file
